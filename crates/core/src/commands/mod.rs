...
use nix_compat::{log::{AT_NIX_PREFIX, Field, LogMessage, ResultType, VerbosityLevel}, store_path::StorePath};
...
fn drv_path_to_build_name(drv_path: &[u8]) -> Arc<String> {
    let store_path = match StorePath::<&str>::from_absolute_path(drv_path) {
        Err(err) => {
